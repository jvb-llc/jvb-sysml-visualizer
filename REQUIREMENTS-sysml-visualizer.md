...
*   **F-1 (Data Loading):** The `jvb-sysml-visualizer` shall load SysML data from an intermediate data format (e.g., JSON) generated by the `jvb-sysml-analyzer`.
...
*   **A-2 (Data Interface):** The data interface between the `jvb-sysml-analyzer` and the `jvb-sysml-visualizer` shall be a clearly defined and versioned JSON schema.
